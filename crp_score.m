function [mean_crps,crps_value,num_nan] = crp_score(fcst,obs,type_cdf,method)
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%
%% The Continous Ranked Probability Score is a strictly proper quadratic measure of   %%
%% the difference between the forecast cdf and the empirical cdf of the observation   %%
%% This empirical CDF of the observation is taken as a step function at the observed  %%
%% value                                                                              %%
%%                                                                                    %%
...
%%          obs = rand(1000,1);                                                       %%
%%          [meanCRPS,crps_values,num_nan] = crp_score(fcst,obs,'weib');              %%
%% Note: the CRPS is positively-oriented!! (larger is better)                         %%
%%                                                                                    %%
%% Based on Durga Lal Shrestha (2024). Continuous rank probability score              %%
...
if ischar(type_cdf)
    switch type_cdf
        ...
        case 'bern'
            p1 = 0.3; p2 = 0.4;
        otherwise
            error('crp_score:Wrong_Empirical_CDF','Unknown empirical CDF');
    end
else
    if numel(type_cdf)~=2
...
crps_value = nan(n,1);      % initialize crps values
            ...
            if  ~missingFcst && ~missingObs
                ...
                if ~isempty(ind) % i.e. obs(i) >  fcst(i,1)
                    ...
                    if obs(t) < fcst(t,end) % right of the observation
                        ...
                        crps_value(t) = crpsLeft + crpsRight + ...
                        ...
                        crps_value(t)  =  crps_right_outside + crpsLeft;
                    end
                    ...
                    crps_value(t)  =  crps_left_outside + crpsRight;
                end
            end % not missing
        end % all forecasts
